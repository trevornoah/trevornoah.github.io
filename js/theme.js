...
// ---------------------------------------------------------------------------------------------------------------------------->
function loadMasonryElements() {
  var BASE_URL="https://www.googleapis.com/youtube/v3/";
  var EMBED_URL="https://www.youtube.com/embed/"
  var API_CALL="playlistItems?part=snippet"
  var API_KEY="AIzaSyCl3WsWOnCruEGiZ1N9v8F12CviuFrh0xQ";
  var PLAYLIST_ID="PL2Rl3eOQecWpzKq8X1zT71rv-ayAOgCBG";
  var MAX_RESULTS=50;

  $.get(BASE_URL + API_CALL + "&playlistId=" + PLAYLIST_ID + "&maxResults=" + MAX_RESULTS + "&key=" + API_KEY, function(data) {
    console.dir(data.items);
    for (var i = 0; i < data.items.length; i++) {
      var snippet = data.items[i].snippet;
      if (snippet.thumbnails.maxres != null) {
        var filter = "";
        if (snippet.title.indexOf("Daily Show") >= 0) {
          filter = "dailyshow";
        } else if (snippet.title.indexOf("Trailer") >= 0) {
          filter = "film";
        } else {
          filter = "standup";
        }
        var context = {name: snippet.title, filter: filter, thumbnail: snippet.thumbnails.maxres.url, url: EMBED_URL + snippet.resourceId.videoId};
        $("#work .container-grid").append(Handlebars.templates.video(context));
      }
    }
